...
class CommitteeAdminTest(TestCase):
    def setUp(self):
        ...
        change_committee_permission = Permission.objects.get(
            codename="change_committee"
        )
        self.user.user_permissions.add(
            view_committee_permission, change_committee_permission
        )
        self.user.save()
    ...
    def test_committee_admin_fields(self):
        ...
        response = self.client.get(
            reverse("admin:committees_committee_change", args=[self.committee.id])
        )
        self.assertContains(response, "Test Committee")
