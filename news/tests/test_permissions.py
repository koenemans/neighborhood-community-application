...
class NewsPermissionsTest(TestCase):
    def setUp(self):
        # Create staff user
        self.user = User.objects.create_user(username="staff_user", email="regular@example.com", password="password")
    ...
    def test_staff_user_cannot_edit_other_committee_post(self):
        ...
        self.assertNotEqual(response.status_code, 403)  # Either 200 (can view) or 302 (redirect)